...
public class AnimatedSprite extends TiledSprite {
	...
	public AnimatedSprite(final float pX, final float pY,
	...
	public AnimatedSprite(final float pX, final float pY,
		...
		super(pX, pY, pTiledTextureRegion, pVertexBufferObjectManager,
				DrawType.DYNAMIC, pShaderProgram);
	}
	...
	public AnimatedSprite(final float pX, final float pY, final float pWidth,
			final float pHeight, final ITiledTextureRegion pTiledTextureRegion,
			final VertexBufferObjectManager pVertexBufferObjectManager) {
		super(pX, pY, pWidth, pHeight, pTiledTextureRegion,
				pVertexBufferObjectManager, DrawType.DYNAMIC);
	}
			...
			if (!this.mAnimationStartedFired && (this.mAnimationProgress == 0)) {
				...
				if (this.mAnimationListener != null) {
					...
					this.mAnimationListener.onAnimationFrameChanged(this,
							AnimatedSprite.FRAMEINDEX_INVALID, 0);
				}
			}
	...
	public static interface IAnimationListener {
		 ...
		 *            is {@link AnimatedSprite#LOOP_CONTINUOUS} when
		 *            {@link AnimatedSprite} loops infinitely.
		 */
		 ...
		 *            equals {@link AnimatedSprite#FRAMEINDEX_INVALID}, the
		 *            first time
		 *            {@link IAnimationListener#onAnimationFrameChanged(AnimatedSprite, int, int)}
		 *            is called.
		 * @param pNewFrameIndex
		 *            the new frame index of the currently active animation.
		 */
		public void onAnimationFrameChanged(
				final AnimatedSprite pAnimatedSprite, final int pOldFrameIndex,
